import type { EntityConfig, ClientFieldsOptions, GeneralConfig } from '../tsTypes';

import composeChildActionSignature from '../types/composeChildActionSignature';

type ClientFieldsOptionsWithChildArgs = ClientFieldsOptions & {
  currentChild: string;
  childArgs: {
...
const getNameOrAlias = (nameWithAlias: string) => nameWithAlias.split(': ')[0];

const composeChildArgs = (
  currentChild: string,
  ...
  composeChildActionSignature(entityConfig, generalConfig, 'childEntities')
    .split(', ')
    .filter(Boolean)
    .reduce<Record<string, any>>((prev, item) => {
      const [argName, argType] = item.split(': ');
      prev[`${currentChild}_${argName}`] = argType; // eslint-disable-line no-param-reassign
      return prev;
    }, {});

const composeArrArgs = (currentChild: string) => ({ [`${currentChild}_slice`]: 'SliceInput' });

const composeChildArgsStr = (childArgs: Record<any, any>) =>
...
const composeFieldsWithChildArgs = (
  ...
  const { childArgs, currentChild, depth, include, exclude, shift } = options as any;
...
const composeFields = (
  ...
  const childArgs: Record<string, any> = {};
