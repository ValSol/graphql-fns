...
type Result = {
  [derivativeName: string]: DerivativeAttributes
};
...
const composeDerivative = (derivativeAttributesArray: Array<DerivativeAttributes>): Result => {
  const derivativeKeys = derivativeAttributesArray.reduce<Array<any>>((prev, item) => {
  ...
  const result = derivativeAttributesArray.reduce<Record<string, any>>((prev, rawItem) => {
