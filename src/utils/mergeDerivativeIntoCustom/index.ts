...
const mergeDerivativeIntoCustom = (
    ...
    Object.keys(action).forEach((name) => {
      ...
      Object.keys(allEntityConfigs).forEach((entityName) => {
        ...
        if (customConfig.type === 'virtual') {
          const involvedOutputEntityNames = Object.keys(involvedEntityNames).reduce<Array<any>>((prev, involvedEntityKey) => {
            if (
  ...
  const Query = Object.keys(derivative).reduce<Record<string, any>>((prev, derivativeKey) => {
  ...
  const Mutation = Object.keys(derivative).reduce<Record<string, any>>((prev, derivativeKey) => {
    const { allow } = derivative[derivativeKey];
    const allowedMethods = getAllowedMethods(allow);
