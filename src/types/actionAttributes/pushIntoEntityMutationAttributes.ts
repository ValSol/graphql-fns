...
const actionName = (baseName: string, derivativeKey: string = ''): string => `pushInto${baseName}${derivativeKey}`;
...
const actionAllowed = (entityConfig: EntityConfig): boolean => entityConfig.type === 'tangible' && Boolean(createPushIntoEntityInputType(entityConfig)[1]);
