...
import createEntityFilterConnectionResolver from '.';
...
beforeAll(async () => {
  const dbURI = 'mongodb://127.0.0.1:27017/jest-entity-filter-connection-type';
...
describe('createEntityFilterConnectionResolver', () => {
  ...
  test('should create type entity resolver', async () => {
    ...
    const restaurantFilterConnection = createEntityFilterConnectionResolver(
      restaurantConfig,
      generalConfig,
      serversideConfig,
    );
    ...
    expect(restaurantFilter2.edges[0].node.title).toBe('Belle Vue');

    expect(restaurantFilter2.edges[1].node.title).toBe('La Cosmopolit');
  });
});
