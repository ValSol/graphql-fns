...
describe('modifyConnectionArgsAndInvolvedFilters util', () => {
  ...
  test('get results for { first: 6 }', () => {
    const args = { first: 6 };
    const involvedFilters: InvolvedFilters = { inputOutputFilterAndLimit: [[], 13] };

    const expectedResult = [{ first: 6 }, { inputOutputFilterAndLimit: [[]] }];
  ...
  test('get results for { first: 6 }', () => {
    const args = { first: 6 };
    const involvedFilters: InvolvedFilters = { inputOutputFilterAndLimit: [[], 3] };

    const expectedResult = [{ first: 3 }, { inputOutputFilterAndLimit: [[]] }];

    const result = modifyConnectionArgsAndInvolvedFilters(args, involvedFilters, name);

    expect(result).toEqual(expectedResult);

    const involvedFilters2: InvolvedFilters = { inputOutputFilterAndLimit: [[]] };
  ...
  test('get results for { first: 5, after: "someCursor" }', () => {
    ...
    const expectedResult = [{ first: 3, after: 'someCursor' }, { inputOutputFilterAndLimit: [[]] }];
  ...
  test('get results for { last: 5, before: "someCursor" }', () => {
    ...
    const expectedResult = [{ last: 3, before: 'someCursor' }, { inputOutputFilterAndLimit: [[]] }];
  ...
  test('get results for { first: 6 }', () => {
    ...
    const expectedResult = [{ last: 3 }, { inputOutputFilterAndLimit: [[]] }];
